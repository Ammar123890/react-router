...
`
      ...
      _optionalChain([onComplete, 'optionalCall', _75 => _75()]);
      controller.close();
    }
  });
  return readable;
  ...
  let dataStrategy = getSingleFetchDataStrategyImpl(
    ...
    (match) => {
      ...
      return {
        ...
        hasShouldRevalidate: Boolean(_optionalChain([routeModule, 'optionalAccess', _76 => _76.shouldRevalidate]))
      };
    },
      ...
      for (let match of matches) {
        if (match.route.id in fetchedData.routes) {
          ...
          if ("error" in routeResult) {
            ...
            if (_optionalChain([results, 'access', _77 => _77[match.route.id], 'optionalAccess', _78 => _78.result]) == null) {
              results[match.route.id] = {
                type: "error",
...
async function fetchAndDecodeViaTurboStream(args, basename, trailingSlashAware, targetRoutes) {
  ...
  try {
    ...
    if (request.method === "GET") {
      ...
      let typed = decoded.value;
      let routeId = _optionalChain([targetRoutes, 'optionalAccess', _79 => _79[0]]);
...
Please change the parent <Route path="${parentPath}"> to <Route path="${parentPath === "/" ? "*" : `${parentPath}/*`}">.`
  ...
  if (locationArg) {
    ...
    invariant(
      parentPathnameBase === "/" || _optionalChain([parsedLocationArg, 'access', _80 => _80.pathname, 'optionalAccess', _81 => _81.startsWith, 'call', _82 => _82(parentPathnameBase)]),
...
function _renderMatches(matches, parentMatches = [], dataRouterState = null, onErrorHandler = null, future = null) {
  ...
  let errors = _optionalChain([dataRouterState, 'optionalAccess', _83 => _83.errors]);
  if (errors != null) {
    let errorIndex = renderedMatches.findIndex(
      (m) => m.route.id && _optionalChain([errors, 'optionalAccess', _84 => _84[m.route.id]]) !== void 0
    );
    invariant(
      errorIndex >= 0,
  ...
  let onError = dataRouterState && onErrorHandler ? (error, errorInfo) => {
    onErrorHandler(error, {
      ...
      params: _nullishCoalesce(_optionalChain([dataRouterState, 'access', _85 => _85.matches, 'optionalAccess', _86 => _86[0], 'optionalAccess', _87 => _87.params]), () => ( {})),
...
function useRouteError() {
  ...
  return _optionalChain([state, 'access', _88 => _88.errors, 'optionalAccess', _89 => _89[routeId]]);
}
function useAsyncValue() {
  let value = React3.useContext(AwaitContext);
  return _optionalChain([value, 'optionalAccess', _90 => _90._data]);
}
function useAsyncError() {
  let value = React3.useContext(AwaitContext);
  return _optionalChain([value, 'optionalAccess', _91 => _91._error]);
}
var blockerId = 0;
...
function useRoute(...args) {
  ...
  return {
    ...
    actionData: _optionalChain([state, 'access', _92 => _92.actionData, 'optionalAccess', _93 => _93[id]])
  };
}
...
function getKeyedLinksForMatches(matches, routeModules, manifest) {
  let descriptors = matches.map((match) => {
    ...
    return [
      ...
      _optionalChain([module, 'optionalAccess', _94 => _94.links, 'optionalCall', _95 => _95()]) || []
    ];
  }).flat(2);
...
function getNewMatchesForLinks(page, nextMatches, currentMatches, manifest, location, mode) {
  ...
  let matchPathChanged = (match, index) => {
    return (
      ...
      // e.g. /files/images/avatar.jpg -> files/finances.xls
      _optionalChain([currentMatches, 'access', _96 => _96[index], 'access', _97 => _97.route, 'access', _98 => _98.path, 'optionalAccess', _99 => _99.endsWith, 'call', _100 => _100("*")]) && currentMatches[index].params["*"] !== match.params["*"]
    );
  };
  if (mode === "assets") {
    ...
    return nextMatches.filter((match, index) => {
      ...
      if (match.route.shouldRevalidate) {
        let routeChoice = match.route.shouldRevalidate({
          ...
          currentParams: _optionalChain([currentMatches, 'access', _101 => _101[0], 'optionalAccess', _102 => _102.params]) || {},
...
function createClientRoutes(manifest, routeModulesCache, initialState, ssr, isSpaMode, parentId = "", routesByParentId = groupRoutesByParentId(manifest), needsRevalidation) {
  return (routesByParentId[parentId] || []).map((route) => {
    ...
    if (routeModule) {
      ...
      let initialData = hasInitialData ? _optionalChain([initialState, 'optionalAccess', _103 => _103.loaderData, 'optionalAccess', _104 => _104[route.id]]) : void 0;
      let hasInitialError = initialState && initialState.errors && route.id in initialState.errors;
      let initialError = hasInitialError ? _optionalChain([initialState, 'optionalAccess', _105 => _105.errors, 'optionalAccess', _106 => _106[route.id]]) : void 0;
      let isHydrationRequest = needsRevalidation == null && (_optionalChain([routeModule, 'access', _107 => _107.clientLoader, 'optionalAccess', _108 => _108.hydrate]) === true || !route.hasLoader);
...
function useFogOFWarDiscovery(router, manifest, routeModules, ssr, routeDiscovery, isSpaMode) {
  React6.useEffect(() => {
    ...
    _optionalChain([window, 'access', _109 => _109.navigator, 'optionalAccess', _110 => _110.connection, 'optionalAccess', _111 => _111.saveData]) === true) {
      return;
    }
...
async function fetchAndApplyManifestPatches(paths, errorReloadPath, manifest, routeModules, ssr, isSpaMode, basename, manifestPath, patchRoutes, signal) {
  ...
  try {
    ...
    if (_optionalChain([signal, 'optionalAccess', _112 => _112.aborted])) return;
    throw e;
  }
...
function PrefetchPageLinksImpl({
  ...
  let dataHrefs = React7.useMemo(() => {
    ...
    nextMatches.forEach((m) => {
      ...
      if (!newMatchesForData.some((m2) => m2.route.id === m.route.id) && m.route.id in loaderData && _optionalChain([routeModules, 'access', _113 => _113[m.route.id], 'optionalAccess', _114 => _114.shouldRevalidate])) {
        foundOptOutRoute = true;
...
function Meta() {
  ...
  for (let i = 0; i < _matches.length; i++) {
    ...
    matches[i] = match;
    if (_optionalChain([routeModule, 'optionalAccess', _115 => _115.meta])) {
  ...
  return /* @__PURE__ */ React7.createElement(React7.Fragment, null, meta.flat().map((metaProps, index) => {
    if (!metaProps) {
      return null;
    }
    ...
    return /* @__PURE__ */ React7.createElement("meta", { key: index, ...metaProps });
  }));
}
...
function Scripts(scriptProps) {
  ...
  let initialScripts = React7.useMemo(() => {
    ...
    let routeModulesScript = !isStatic ? " " : `${_optionalChain([manifest, 'access', _116 => _116.hmr, 'optionalAccess', _117 => _117.runtime]) ? `import ${JSON.stringify(manifest.hmr.runtime)};` : ""}${!enableFogOfWar ? `import ${JSON.stringify(manifest.url)}` : ""};
...
function BoundaryShell({
  ...
  if (_optionalChain([routeModules, 'access', _118 => _118.root, 'optionalAccess', _119 => _119.Layout]) && !isOutsideRemixApp) {
    return children;
  }
...
function createMemoryRouter(routes, opts) {
  return createRouter({
    basename: _optionalChain([opts, 'optionalAccess', _120 => _120.basename]),
    getContext: _optionalChain([opts, 'optionalAccess', _121 => _121.getContext]),
    future: _optionalChain([opts, 'optionalAccess', _122 => _122.future]),
    history: createMemoryHistory({
      initialEntries: _optionalChain([opts, 'optionalAccess', _123 => _123.initialEntries]),
      initialIndex: _optionalChain([opts, 'optionalAccess', _124 => _124.initialIndex])
    }),
    hydrationData: _optionalChain([opts, 'optionalAccess', _125 => _125.hydrationData]),
    routes,
    hydrationRouteProperties,
    mapRouteProperties,
    dataStrategy: _optionalChain([opts, 'optionalAccess', _126 => _126.dataStrategy]),
    patchRoutesOnNavigation: _optionalChain([opts, 'optionalAccess', _127 => _127.patchRoutesOnNavigation]),
    unstable_instrumentations: _optionalChain([opts, 'optionalAccess', _128 => _128.unstable_instrumentations])
  }).initialize();
}
var Deferred2 = class {
  constructor() {
    ...
    (newState, { deletedFetchers, newErrors, flushSync, viewTransitionOpts }) => {
      if (newErrors && onError) {
        Object.values(newErrors).forEach(
          (error) => onError(error, {
            ...
            params: _nullishCoalesce(_optionalChain([newState, 'access', _129 => _129.matches, 'access', _130 => _130[0], 'optionalAccess', _131 => _131.params]), () => ( {})),
      ...
      if (reactDomFlushSyncImpl && flushSync) {
        reactDomFlushSyncImpl(() => {
          if (transition) {
            _optionalChain([renderDfd, 'optionalAccess', _132 => _132.resolve, 'call', _133 => _133()]);
      ...
      if (transition) {
        _optionalChain([renderDfd, 'optionalAccess', _134 => _134.resolve, 'call', _135 => _135()]);
        transition.skipTransition();
        setInterruption({
          state: newState,
          currentLocation: viewTransitionOpts.currentLocation,
  ...
  let navigator = React9.useMemo(() => {
    return {
      ...
      replace: (to, state2, opts) => router.navigate(to, {
        replace: true,
        state: state2,
        preventScrollReset: _optionalChain([opts, 'optionalAccess', _137 => _137.preventScrollReset])
      })
    };
  }, [router]);
    ...
    (error, errorInfo) => {
      if (dataRouterContext && dataRouterContext.onError && dataRouterStateContext) {
        dataRouterContext.onError(error, {
          ...
          params: _optionalChain([dataRouterStateContext, 'access', _138 => _138.matches, 'access', _139 => _139[0], 'optionalAccess', _140 => _140.params]) || {},
  ...
  render() {
    ...
    if (!(resolve instanceof Promise)) {
      ...
      promise = resolve.then(
        ...
        (error) => {
          _optionalChain([this, 'access', _141 => _141.props, 'access', _142 => _142.onError, 'optionalCall', _143 => _143(error)]);
